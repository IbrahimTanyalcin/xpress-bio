...
\textbf{(g)} Users can open the "g-nome" app on the side bar to connect to public LLM models with their API keys. Models get access to currently visible sequence data along with any BLAST windows and tracks. Xpress-Bio provides a mechanism for these LLM models to propose code snippets that can be run by the client which allows manipulation of visual elements on the screen. This allows scientists to extend UI functionality like sorting BLAST outputs and searching keywords within files.\label{fig:fig1}](./image1.png){ width=100% }
