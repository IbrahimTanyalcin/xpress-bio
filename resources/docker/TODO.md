...
- 2026 Jan
  - Implement smooth animated thinking blocks via `<details>` elements using the sibling approach in [this](https://codepen.io/IbrahimTanyalcin/pen/RNRGOWG) codepen. This technique uses `interpolate-size`, which is not yet supported accross all browsers. For this to work, you will need to update promptParser's related sse event types to send the correct html markup via `server-g-nome-promptext`. Additionally, you will need to inject the `css` part in `@chatGPT/js/init.js`. Applicable to claude's thinking and openai's reasoning models.