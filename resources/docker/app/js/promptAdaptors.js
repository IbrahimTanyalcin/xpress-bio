const 
    ...
    {promptParser1, promptParser2, promptParser3} = require("./promptParser.js");
exports.promptAdaptors = (wk, {sysPrompt = `Default System Prompt`, separator = defSeparator} = {sysPrompt: defSysPrompt, separator: defSeparator}) => {
    ...
    function start3 ({model, prompt, meta, store, stream}){
        return `{
            "model": "${model}",
            "max_tokens": 16384,
            "stream": ${stream},
    ...
    async function send3 ({instance, prompt, meta, window_id}){
        const {ledger, oWS, ws, model, uri} = wk.get(instance);
        ws.msg({
            channel: oWS.name,
            sessid: oWS.sessid,
        ...
        const res = await fetch(
            uri,
            {
                method: 'POST',
                headers: {
                    "x-api-key": `${ledger.tokens.get(model)}`,
        ...
        ws.logIfDebug("FETCH WORKED!");
        //you can do log: true below to force logging
        const parser = promptParser3({instance, adaptorsModelBody : adaptors[model].body, ws, oWS, window_id, log: ws.logIfDebug});
        for await (const chunk of res.body) {
            parser.feed(chunk);
        }
        ...
        let term;
        if (res?.headers?.get("anthropic-ratelimit-tokens-remaining")) {
            term = `\n\n  [${
                res?.headers?.get?.("anthropic-ratelimit-tokens-remaining") ?? "unknown"
            } tokens remaining. Reset at ${
                res?.headers?.get?.("anthropic-ratelimit-tokens-reset") ?? "unknown"
            } (RFC 3339) End]`;
        } else {
            term = `\n\n  [End]`;
        }
