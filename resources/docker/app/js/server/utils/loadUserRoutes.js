...
module.exports = async function ({express, app, info, files, serverSent, memcache, ws}) {
    ...
    return Promise.all(routeFiles.map(routeFile => require("./" + routeFile)({express, app, info, files, serverSent, memcache, ws})))
    .then(() => routeFiles);
}